...
```bash
$ helm install vswh --namespace vswh --wait oci://ghcr.io/bank-vaults/helm-charts/berglas-webhook --create-namespace
```
...
```bash
$ helm upgrade --namespace vswh --install vswh oci://ghcr.io/bank-vaults/helm-charts/berglas-webhook --set-string securityContext.runAsUser="" --create-namespace
```
...
| `image.repository` | string | `"ghcr.io/bank-vaults/berglas-webhook"` | Container image repo that contains the admission server |
...
| `service.name` | string | `"berglas-webhook"` | Webhook service name |
